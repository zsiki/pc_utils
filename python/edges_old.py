...
def ransac_line(points, tolerance=0.1, rep=0, min_dist=0.5):
    """ find RANSAC line/edge in a point cloud
        ...
        :param min_dist: minimal distance between random samples
        :returns: TODO
    """
    if rep < 1:
    ...
    for _ in range(rep):
        ...
        while k < n // 2:
            ...
            if np.sum(np.square(p1[0] - p1[1])) > min_dist:
                break
            k += 1
        else:
            return None, None, 0
...
def get_edges(pc, ext=5, threshold=0.1, limit=30, edge_limit=90, d1=0.2, d2=0.2, debug=False):
    ...
    for i, corner1 in enumerate(corners):
        for corner2 in corners[i+1:]:
            ...
            if debug:
                print(f"{n} wall points in poly")
                print(f"{m} corner points in poly")
                print(f"{n / area:10.3f} > {edge_limit:10.3f}")
            ...
            if n/area > edge_limit: # and m < 4: # m == 0:         # number or points in 1 m^2
                k += 1
                edges.append([corner1, corner2])
                if debug:
                    print(k," edge: ", corner1, corner2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--extend', type=float, default=10.0,
                        help='Extent the size of minimax to limit corners, default: 10')
    parser.add_argument('-l', '--limit', type=int, default=90,
                        help='minimal number of points limit for edges, default: 90')
    parser.add_argument('-i', '--edgelimit', type=int, default=90,
                        help='minimal number of points limit for RANSAC line, default: 90')
    parser.add_argument('-c', '--cornerdist', type=float, default=0.2,
                        help='Threshold distance from corners in edge detection, default: 0.2')
    parser.add_argument('-g', '--edgedist', type=float, default=0.2,
                        help='Threshold distance from edges in edge detection, default: 0.2')
